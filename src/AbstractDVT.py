# ----------------------------------------------------
# Abstract istance Vector Routing Table
# ----------------------------------------------------
...
def getAbstractDVTString(self, node):
	nodeID = node.nodeID
	abstractDVTStr = '#' + node.nodeID + '#'
	nodesTo = node.dvt.keys()
	for i in range(0, len(nodesTo)):
		abstractDVTStr += nodesTo[i] + '=' + node.dvt[nodesTo[i]][0]	# cost is the first entry in tuple
		if (i < len(nodesTo) - 1):
			dvtEntryStr += ','
